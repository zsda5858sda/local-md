...
- 自訂附註連結：輸入顯示文字與附註內容，點擊後以小標籤閱讀，內容隨 Markdown 文件保存。
...
- 桌面版啟動時會自動開啟最近一次成功使用的工作區；資料夾已不存在或無法存取時，會停留在選擇工作區畫面。
...
- 點選工具列「插入圖片」、直接貼上圖片或把圖片檔拖進編輯器，圖片都會存到同層的 `assets/` 資料夾，Markdown 只記錄相對路徑。
- 支援 PNG、JPEG、GIF、WebP、SVG、BMP，單張原始圖片上限 20 MB；可一次拖入多張圖片。
- 「連結」分頁插入的遠端圖片仍是網址，不會下載到工作區。
- 圖片右上角的「並排」可將圖片設為半寬；連續插入兩張半寬圖片時會並排在同一行。
- 圖片的「說明」會儲存為 Markdown 替代文字，讓讀取文件的工具也能取得描述。
- 可把 `.md` 交給支援解析 data URI 的 AI 工具解碼圖片；僅讀取文字的 AI 不一定能辨識圖片內容。需要時請同時提供原始圖片，並在文件中補充文字描述。

### 附註

1. 選取要附加附註的文字，或將游標放在插入位置。
2. 點選工具列「插入附註」，填寫顯示文字與附註內容；附註內容可自行輸入或貼上多行文字。
3. 確認後，直接點擊文件中的顯示文字，即可查看附註小標籤。點擊外部、關閉按鈕或按 `Escape` 關閉。
4. 將游標放在既有附註內，再點選「插入附註」，即可修改內容或移除附註；移除時保留顯示文字。

附註內容由使用者提供，隨文件一起儲存，重新開啟仍可閱讀。小標籤是 Local MD 的功能，其他 Markdown 閱讀器會以一般連結顯示。
